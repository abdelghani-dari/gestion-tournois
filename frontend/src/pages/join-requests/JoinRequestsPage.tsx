...
import { TeamLogoNameCell } from "../../components/statistics/StatisticTableCells";
    ...
    <>
      ...
      <PageStack>
        {!authLoading && !isAuthenticated ? (
          ...
          <>
              <ComponentCard title="Envoyer une demande" desc="Équipe vers tournoi accepté">
                      ...
                      {requestsPagination.paginatedItems.map((request) => (
                        <tr key={request.id} className={clsx("transition-colors", t.tableRow, t.navHover)}>
                          ...
                          <td className="px-4 py-3">
                            <TeamLogoNameCell
                              name={request.team?.short_name || requestTeamName(request, myTeams)}
                              logoPath={request.team?.logo_path ?? myTeams.find((team) => team.id === request.team_id)?.logo_path}
                            />
                          </td>
                          ...
                          <td className="px-4 py-3">
                            {request.status === "pending" && (user?.role === "admin" || (request.tournament?.created_by != null && Number(request.tournament.created_by) === Number(user?.id))) ? (
            ...
            >
              <form onSubmit={handleCreateRequest} className="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <FormSearchableSelect
                  ...
                  disabled={submitting || loading}
                  options={teamSelectOptions(availableTeams, user?.id)}
                />
                {form.tournament_id && availableTeams.length === 0 && !loading && (
                  <p className={clsx("md:col-span-2 text-sm", t.textMuted)}>Aucune équipe disponible pour cette demande.</p>
                )}
